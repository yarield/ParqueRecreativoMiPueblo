...
export default function ClienteHistorialDialog({ open, onClose, cliente }: ClienteHistorialDialogProps) {
  ...
  // Al abrir el diálogo con otro cliente el historial es otro: se empieza de la
  // primera página.
  const facturas = useMemo(() => data?.facturas ?? [], [data])
  const { items: facturasPagina, control: paginacion } = usePaginacion(facturas, [cliente?.id])
